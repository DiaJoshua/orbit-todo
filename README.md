...
- **Liquid Glass everywhere** — every card, row, column and tile is translucent glass over the ambient glow, with specular top edges. Three intensities in Settings: Off, Frosted (default, cheap to render), and Max (real backdrop blur on every card).
- **Settings hub — everything is toggleable** — ⚙ Settings covers Appearance (theme/accent/glass/density/reduce-motion), Features (confetti, sounds, haptics, voice, tips, quotes, streak flame, swipe actions, mascot — each with its own switch), Dashboard tiles (show/hide any tile), Behavior (start view, quick-capture space, focus length, daily focus goal, reminders), Privacy, Data, and About.
- **iPhone-grade touches** — privacy blur the moment you switch away; synthesized completion/focus chimes (no audio files); swipe a task right to complete or left to delete on phones; Spotlight-style ⌘K that finds tasks and notes, not just commands; and a live focus countdown in the tab title.
