...
- **Liquid Glass with a living atmosphere** — a four-blob color field drifts behind the app (one blob follows your accent color), and every card, row, column and tile is translucent glass over it with specular top edges. Three genuinely different levels in Settings: **Off** (solid, calm), **Frosted** (the atmosphere glows through every surface), **Max** (more transparent + real backdrop blur and saturation on every card).
- **Five interface styles** — Settings → Style switches the whole design language: 💎 Liquid Glass (translucent, atmospheric), 🍃 Zen (flat, airy, Things-style), 📐 Pro (crisp and dense, Linear-style), ⚡ Pop (neubrutalist ink borders and hard shadows), 📜 Journal (warm paper with serif headings). Your accent color recolors buttons, highlights and the background atmosphere in one move — mix any style, theme, accent and glass level.
- **Settings hub — everything is toggleable** — ⚙ Settings covers Appearance (theme/accent/glass/density/reduce-motion), Features (confetti, sounds, haptics, voice, tips, quotes, streak flame, swipe actions, mascot — each with its own switch), Dashboard tiles (show/hide any tile), Behavior (start view, quick-capture space, focus length, daily focus goal, reminders), Data, and About — no filler toggles, everything visibly does something.
- **A full sound design** — every action speaks with its own synthesized sound (no audio files, fully offline): adds blip, completions chime, un-checks descend, deletes thud, undo rises, drags thock, navigation ticks, toggles click, sheets tap open and closed, focus has start/pause/resume/finish tones, milestones sparkle, errors buzz. One switch turns it all off; a volume setting (Quiet/Normal/Loud) previews itself.
- **iPhone-grade touches** — swipe a task right to complete or left to delete on phones; Spotlight-style ⌘K that finds tasks and notes, not just commands; and a live focus countdown in the tab title.
- **Apps hub** — your other web apps live inside Orbit. Sipit (the trip money tracker) comes built in: open it from the sidebar, and it keeps running with its state intact while you flip between views. On wide screens hit ◫ to pin it beside Orbit and use both at the same time. Add any website as an app with ＋ (the list syncs to your devices), or deep-link with `?app=trip`.
